...
def update_table(sec):
    for i in range(days):
        for j in range(periods):
            ...
            cursor = conn.execute(f"SELECT SUBCODE, FINI, SUBTYPE FROM SCHEDULE\
                WHERE DAYID={i} AND PERIODID={j} AND SECTION='{sec}'")
            cursor = list(cursor)

            butt_grid[i][j]['bg'] = '#a7aeb1'
            if len(cursor) != 0:
...
def process_button(d, p, sec):
    ...
    tk.Label(details, text='Faculty Name: '+fname, font=('comic'), anchor="w").pack(expand=1, fill=tk.X, padx=20)
    tk.Label(details, text='Faculty Email: '+femail, font=('comic'), anchor="w").pack(expand=1, fill=tk.X, padx=20)
